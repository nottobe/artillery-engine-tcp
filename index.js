...
TCPEngine.prototype.step = function step(rs, ee, opts) {
  ...
  if (rs.send) {
    return function send(context, callback) {
      const payload =
        typeof rs.send.payload === "object"
          ? JSON.stringify(rs.send.payload)
          : rs.send.payload;
...
TCPEngine.prototype.compile = function compile(tasks, scenarioSpec, ee) {
  ...
  return function scenario(initialContext, callback) {
    const init = function init(next) {
      initialContext.client = net.createConnection(
        self.script.config.tcp.port,
        self.script.config.target
      );
      ee.emit("started");
